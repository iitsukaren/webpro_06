...
- [ ] 開発者向けドキュメントの作業
  ...
  - [ ] データ構造の決定(データに何を入れるか) :thinking:
  - [ ] ページ構造の検討(ページ内の情報) :thinking:
  - [ ] ページ遷移の検討(リンク毎の繋がりだけでなくリンクを用意するのか項目名をクリックするのかまで書く) :thinking:
  - [ ] HTTPメソッドとリソース名の決定(getかpostどちらを使う)
  - [ ] ページ遷移図の作成 :writing_hand:
  - [ ] ページ構造の決定
  - [ ] ドキュメントの構成の検討(仕様書の構造を考える) :thinking:
  - [ ] 概要 :writing_hand:
  - [ ] HTTPメソッドとリソース名一覧 :writing_hand:
  - [ ] データ構造 :writing_hand:
  - [ ] リソース名ごとの機能の詳細(ファイル毎の機能説明) :writing_hand:
- [ ] 管理者向けドキュメントの構成
...
- [ ] 利用者向けドキュメントの作業
  - [ ] 構成の検討 :thinking:
  - [ ] スクリーンショットの保存と整理(スクショだけでなく枠で囲む，線を引くなどわかりやすくする工夫が求められる) :computer:
  - [ ] 概要 :writing_hand:
